...
export function rollMacro({
  ...
  const marketReturnAnnual = clamp(0.08 - inflationAnnual * 0.7 - recessionProbAnnual * 0.12, -0.25, 0.15);
...
export function generateEvent({
  ...
  if (creditThin) {
    candidates.push({
      ...
      left: {
        ...
        bullets: ["Builds history over time", "Low utilization target (<10%)", "Requires discipline every month"],
      },
      right: {
        id: "right",
        title: "Decline (cash-only)",
        bullets: ["No revolving-debt risk", "Score grows slower", "Loan terms may be worse later"],
      },
    });
  }
  ...
  candidates.push({
    ...
    left: {
      ...
      bullets: ["Monthly contributions", "Long horizon compounds", "Drawdowns happen but are survivable with cash buffer"],
    },
    right: {
      id: "right",
      title: "Hold cash until things “feel safe”",
      bullets: ["Less volatility stress", "Opportunity cost during rebounds", "Inflation quietly erodes purchasing power"],
    },
  });
  ...
  if (scenarioFlags.childcareMonthly > 0 || scenarioFlags.remittanceMonthly > 0) {
    candidates.push({
      ...
      left: {
        ...
        bullets: ["Protects obligations", "Stress eases with predictability", "Short-term regret and social friction"],
      },
      right: {
        id: "right",
        title: "Bridge with credit “just this month”",
        bullets: ["Feels painless now", "APR compounds quickly", "Credit score can slip if utilization spikes"],
      },
    });
  }
  ...
  if (highDti) {
    candidates.push({
      ...
      left: {
        ...
        bullets: ["Guaranteed return equals APR", "DTI improves over months", "Stress relief is real and immediate"],
      },
      right: {
        id: "right",
        title: "Minimum payments + keep lifestyle",
        bullets: ["Avoids short-term sacrifice", "Interest drags for years", "Loan options shrink when you need them most"],
      },
    });
  }
