...
async function main() {
  ...
  if (FILTER_TOPIC) {
    ...
    console.log(
      `🔍 Filtered to topic "${FILTER_TOPIC}": ${chunks.length} chunks`,
    );
  }
  ...
  for (let b = 0; b < batches.length; b++) {
    ...
    process.stdout.write(
      `   Batch ${b + 1}/${batches.length} (${batch.length} chunks)... `,
    );
    const embeddings = await embedBatch(batch.map((c) => c.content));
    batch.forEach((c, idx) =>
      embedded.push({ ...c, embedding: embeddings[idx] }),
    );
    console.log("✓");
  }
  ...
  for (let b = 0; b < insertBatches.length; b++) {
    ...
    process.stdout.write(
      `   Insert batch ${b + 1}/${insertBatches.length}... `,
    );
