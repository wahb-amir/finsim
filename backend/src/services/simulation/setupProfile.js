...
function applyCareerModifiers(modifiers, career) {
  ...
  if (/doctor|lawyer|pilot|engineer|accountant/.test(c)) {
    ...
    m.burnoutDriftMonthly = Math.max(
      0.1,
      (m.burnoutDriftMonthly || 0.2) - 0.05,
    );
...
function scaleStartingState(scenarioStart, startSalary) {
  ...
  if (!salary || salary <= 0) {
    return {
      ...scenarioStart,
      debt: scenarioStart.debt
        ? { ...scenarioStart.debt }
        : { kind: "none", balance: 0, apr: 0 },
      investments: scenarioStart.investments
        ? { ...scenarioStart.investments }
        : {},
    };
  }
  ...
  const scaled = {
    ...
    baseExpensesMonthly: Math.round(
      (scenarioStart.baseExpensesMonthly || 1800) * expenseRatio,
    ),
    cash: Math.round(
      (scenarioStart.cash || 800) * Math.min(Math.max(ratio, 0.5), 2.5),
    ),
    investments: scenarioStart.investments
      ? { ...scenarioStart.investments }
      : {},
  ...
  if (scenarioStart.debt?.balance > 0) {
    scaled.debt = {
      ...scenarioStart.debt,
      balance: Math.round(
        scenarioStart.debt.balance * Math.min(Math.max(ratio, 0.6), 1.8),
      ),
    };
  }
...
function buildSetupProfile(
  scenario,
  { startSalary, climateLabel, career } = {},
) {
