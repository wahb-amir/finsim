...
export function MetricCard({ label, value, colorCode, isProgress = false, compact = false }) {
  if (compact) {
    return (
      <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-[#111111] border border-[#1F1F1F]">
        <span className="text-[11px] text-[#6B6B6B] uppercase tracking-wider font-medium leading-tight">{label}</span>
        {isProgress ? (
          <div className="flex items-center gap-2">
            ...
            <span
              ...
              style={{
                color: value < 30 ? "#10B981" : value < 60 ? "#F59E0B" : "#EF4444",
              }}
            >
              {Math.round(value)}
            </span>
  ...
  return (
    <div className="rounded-xl bg-[#111111] border border-[#1F1F1F] p-4 hover:border-[#2A2A2A] transition-colors duration-200">
      <div className="text-[11px] text-[#6B6B6B] uppercase tracking-widest font-medium mb-1.5">{label}</div>
      {isProgress ? (
